...
RUN pnpm install --frozen-lockfile --filter @chat-app/backend
...
# Build backend
RUN pnpm --filter @chat-app/backend build
...
RUN pnpm install --frozen-lockfile --prod --filter @chat-app/backend
